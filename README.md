...
- Each mod record keeps full fields plus `source`, `automated`, and `update_history` (list of ISO-8601 timestamps).
...
Optional fields: `obsolete_reason`, `obsolete_successor`, `obsolete_alternative`, `note`, `other_link`, `sortable`. See `docs/SCHEMA.md`.

- `automated: false` - curated (initial import). Future RSS-discovered mods will be `true` until human review.
...
Dataset facts are from public LoversLab pages. This repo is public domain / CC0 where applicable. Respect LoversLab ToS.
